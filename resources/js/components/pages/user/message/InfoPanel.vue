...
<template>
  <transition name="">
    ...
    >
      <!-- Header -->
      <header class="h-16 flex justify-between items-center p-5 border-b border-gray-200 dark:border-gray-700 sticky top-0 bg-white dark:bg-[#171717] z-10">
        <div class="flex items-center gap-2">
          <ArrowLeft v-if="globalStore.isMobileView" class="w-5 h-5 cursor-pointer hover:text-indigo-500 transition-colors" @click="closePanelInfo" />
          <h1 class="font-semibold text-gray-800 dark:text-white">Thông tin đoạn chat</h1>
        </div>
        <X v-if="!globalStore.isMobileView" class="w-5 h-5 cursor-pointer hover:text-indigo-500 transition-colors" @click="closePanelInfo" />
      </header>
      ...
      <div class="flex-1 overflow-y-auto px-6 py-5 space-y-6">
        ...
        <section class="flex flex-col items-center text-center space-y-3">
          <img :src="getAvatarConversation(chatStore.activeConversation) || AVATAR_DEFAULT" class="w-20 h-20 rounded-full border shadow-sm object-cover" />
        ...
        <section class="border-t border-gray-200 dark:border-gray-700 pt-4">
          ...
          <div class="space-y-2">
            ...
            <button class="w-full text-left px-3 py-2 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition text-red-500">Xóa đoạn chat</button>
          </div>
        </section>
        ...
        <section class="border-t border-gray-200 dark:border-gray-700 pt-4">
          ...
          <div class="flex flex-wrap gap-2">
            <div v-for="i in 8" :key="i" class="w-16 h-16 bg-gray-200 dark:bg-gray-700 rounded-lg flex items-center justify-center text-gray-400">📷</div>
          </div>
        </section>
      </div>
    </aside>
