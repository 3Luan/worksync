...
<template>
  <div class="p-2 rounded-md sm:p-5">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center w-full gap-3 sm:gap-5 mt-1">
      ...
      <div v-if="table.getRowModel().rows?.length" class="flex flex-row justify-center sm:justify-end w-full sm:w-auto gap-3 sm:gap-5">
        <PaginationTable :model-value="paginationValue" />
        <Combobox :options="pageLengthOptions" v-model="paginationValue.perPage" :is-search="false" class="h-8 w-16 dark:border-gray-300" />
      </div>
    </div>
  </div>
</template>
