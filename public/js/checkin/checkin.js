...
setInterval(function () {
    ...
    document.querySelector(".c-dashboard__date").textContent =
        formattedDateTime;

    // Update current time (hour and minute)
    const hourMinute = currentDate.toLocaleTimeString("en-US", {
        hour12: false,
        hour: "2-digit",
        minute: "2-digit",
    });
    document.querySelector(".c-dashboard__hour-minute").textContent =
        hourMinute;
...
function startBreak() {
    ...
    $.ajax({
        ...
        error: function (xhr, status, errorThrown) {
            ...
            var errorMessage = xhr.responseJSON
                ? xhr.responseJSON.error
                : "Failed to start break. Please try again later.";
            toastr.error(errorMessage);
        },
        complete: function () {
            stopLoading(); // Stop loading animation
...
function endBreak() {
    ...
    $.ajax({
        ...
        error: function (xhr, status, errorThrown) {
            ...
            var errorMessage = xhr.responseJSON
                ? xhr.responseJSON.error
                : "Failed to end break. Please try again later.";
            toastr.error(errorMessage);
        },
        complete: function () {
            stopLoading(); // Stop loading animation
